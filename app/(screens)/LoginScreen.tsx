      ...
      try {
        ...
        if (data.message == "Invalid credentials") ToastAndroid.show('Identifiant Introuvable !', ToastAndroid.LONG)
          ...
          if (data.user.accès === "ouvert" && data.user.role === "percepteur") {
            ...
            router.replace('/(screens)/HomeScreen', { print: false }); // Naviguer vers Home après connexion
    ...
    const getUser = async () => {
      try {
        ...
        if (value !== null) {
          ...
          if (JSON.parse(value).role === "percepteur" && JSON.parse(value).accès === "ouvert") router.replace('/(screens)/HomeScreen');
          else if (JSON.parse(value).role === "superviseur" && JSON.parse(value).accès === "ouvert") router.replace('/(screens)/ControlScreen');
        }
      } catch (e) {
