  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <TouchableOpacity style={{ padding: 7 }} onPress={() => router.push({pathname: '/(screens)/HomeScreen', params:{ print: false }})} >
      ...
      {History.length > 0 && <FlatList style={styles.eventScroll} contentContainerStyle={styles.eventContainer}
        ...
        renderItem={({ item, index }) => (
          <TouchableOpacity key={index} style={styles.eventCard} onPress={() => {
            if (item?.plaque != "web") router.push({pathname: '/(screens)/StoryPrintScreen', params: { plaque: item.plaque, date: item.date, name: item.user || 'Inconnu'  } });
          }} >
