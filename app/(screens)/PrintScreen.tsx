...
export default function Screen() {
  ...
  const { plaque, user, len } = useLocalSearchParams();

  const parsedUser = JSON.parse(user); // Attention : try/catch conseillé ici
  ...
  const uploadData = async () => {
    ...
    // const ticketsImprimes = data.length === 0 ? 1 : data.length + 1; // Utilisation de la fonction pour récupérer le nombre de tickets
    // console.log(ticketsImprimes)
    // const montPos = ticketsImprimes * 500; // Calcul du montant
    const newRecord = {
      user_id: parsedUser.id, // ID de l'utilisateur
      Tickets: 1, // Nombre de tickets imprimés
      Ratés: 0, // Nombre de tickets ratés
      Mont_Pos: 500, // Montant calculé
      Mont_Cash: 0, // Montant en espèces
      Transport: 0, // Transport
      Obs: 'en cours', // Observation
    };
    ...
    console.log("Données à envoyer:", newRecord);

    try {
      // Envoi des données au serveur
      const response = await fetch("https://taxeparking.mastagate.com/api/save/rapport", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        // Sauvegarder localement si l'impression a réussi
        const history = await AsyncStorage.getItem("history") || "[]";
        const histo = JSON.parse(history)
        histo.push(forHistory);
        await AsyncStorage.setItem("history", JSON.stringify(histo));
        await AsyncStorage.setItem('dashboard', JSON.stringify(data.data))

        ToastAndroid.show("Impression synchronisation réussi !", ToastAndroid.LONG);
        await _printRecieve(); // Appel de la fonction d'impression
  ...
  const handlePrint = async () => {
    ...
    // Imprimer le reçu
    try {
      const response = await fetch("https://taxeparking.mastagate.com");
      ...
      if (response.ok) { 
        ...
        console.error("Erreur de connexion au serveur:", response);
        ToastAndroid.show("Erreur de connexion au serveur.", ToastAndroid.LONG);
      }
    } catch (error) {
      console.error("Erreur de connexion:", error);
      ToastAndroid.show("Veillez vérifier votre connexion !", ToastAndroid.LONG);
    }
        ...
        <View style={{ ...styles.subscriptionCard2, height: 400 }}>
          ...
          <View style={{ width: '100%', alignItems: 'center', justifyContent: 'center', marginVertical: 10, borderTopWidth: 1, borderBottomWidth: 1, paddingVertical: 20, borderStyle: "dashed" }}>
            ...
            <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between', marginBottom: 0}}>
              ...
              <Text style={styles.subscriptionTitle}> {`CGO-${len}/${plaque.toLocaleUpperCase()}`} </Text>
            </View>

            <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between', marginBottom: 0}}>
              <Text style={{ ...styles.benefitText }}>Date & H : </Text>
              <Text style={styles.subscriptionTitle}> {currentDateTime.toLocaleDateString() + ' ' + currentDateTime.getHours() + ':' + currentDateTime.getMinutes()} </Text>
            </View>
            ...
            <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between', marginBottom: 0}}>
              ...
              <Text style={styles.subscriptionTitle}> {parsedUser.name} </Text>
            </View>
          ...
          <TouchableOpacity style={{ alignSelf: 'center', marginTop: 20, }} onPress={() => router.replace('/(screens)/HomeScreen')}>
